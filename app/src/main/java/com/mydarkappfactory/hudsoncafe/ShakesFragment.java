...
public class ShakesFragment extends android.support.v4.app.Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.layout_frag_shakes, container, false);

        firebaseDb = FirebaseDatabase.getInstance().getReference();
        appetizers = new ArrayList<>();

        RecyclerView recyclerView = view.findViewById(R.id.recycler_view_shakes);
        LinearLayoutManager layoutManager = new LinearLayoutManager(getContext());
        SQLiteOpenHelper dbHelper = new DBHelper(getContext());
        db = dbHelper.getReadableDatabase();

        firebaseDb.child("Menu").child("Shakes").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
...
//                ArrayList<Dish> dishes = dataSnapshot.getValue(t);
                Cursor cursor = db.query("CART_DISHES", new String[]{"_id", "NAME", "QUANTITY", "PRICE"}, "CATEGORY = ?", new String[]{"SHAKE"}, null, null, null);
        ...
        adapter = new RecyclerViewAdapter(appetizers, db, "SHAKE");
