...
public class MaincourseFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.layout_frag_maincourse, container, false);

        firebaseDb = FirebaseDatabase.getInstance().getReference();
        appetizers = new ArrayList<>();

        RecyclerView recyclerView = view.findViewById(R.id.recycler_view_maincourse);
        LinearLayoutManager layoutManager = new LinearLayoutManager(getContext());
        SQLiteOpenHelper dbHelper = new DBHelper(getContext());
        db = dbHelper.getReadableDatabase();

        firebaseDb.child("Menu").child("MainCourse").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
...
//                ArrayList<Dish> dishes = dataSnapshot.getValue(t);
                Cursor cursor = db.query("CART_DISHES", new String[]{"_id", "NAME", "QUANTITY", "PRICE"}, "CATEGORY = ?", new String[]{"MAIN_COURSE"}, null, null, null);
        ...
        adapter = new RecyclerViewAdapter(appetizers, db, "MAIN_COURSE");
